...
export async function getHtmlContent (payload: Payload) {
  const {
    pagesDir, templatePath, pageName, pageTitle, pageEntry, isMPA, data, entry, extraData,
    input, pages, jumpTarget, origin, hasMpaPlugin, hasUnocss
  } = payload;
  let content = '';
  ...
  function getHref(item: string, params?: string) {
    const _params = params ? '?' + params : '';
  	return !hasMpaPlugin ? `${origin}/${pagesDir}/${item}/index.html${_params}` : `${origin}/${item}/index.html${_params}`;
  }
  ...
  if (pageName === 'index') {
    ...
    <script type="module">import 'uno:default.css';</script>\n</body>`);
    } else {
      content = content.replace('</body>', `${links.join('').replace(/,/g, ' ')}\n</body>`);
    }
  } else {
